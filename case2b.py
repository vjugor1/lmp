...
class case2b:
	...
	def fasten_branches(self, alphas=[1.0, 1.0, 1.0, 10000000.0, 0.0], branches=[(14,5), (25, 5), (31, 5), (14, 2), (14, 5)]):
		"""
		Input:
...
#		for tuple_, alpha in zip(branches, alphas):
#			from_ = tuple_[0]
#			to_   = tuple_[1]
#			try:
...
#				print('Branch (', from_, ', ', to_, 'does not exist') 
		alpha = 1.0
		#bound max flow 3 <-> 11
		self.ppc['branch'][14][5] *= alpha
		#bound max flow 9 <-> 16
		self.ppc['branch'][25][5] *= alpha
		#bound max flow 27 <-> 26
		#ppc['branch'][35][5] *= alpha
		#bound max flow 18 <-> 19
		#ppc['branch'][23][5] *= alpha
		#bound max flow 22 <-> 23
		self.ppc['branch'][31][5] *= alpha
		self.ppc['branch'][14][2] = 10000000*alpha
		self.ppc['branch'][14][5] = 0*alpha

		self.ppc['branch'][9][5] *= .5
		self.ppc['branch'][23][5] *= .5
		self.ppc['branch'][30][5] *= .5
		self.ppc['branch'][33][5] *= .5
		self.ppc['branch'][39][5] *= 5
		self.ppc['branch'][9][5] *= 2

		
	def set_costs(self, costs_list = [1000, 1000, 1100, 1200, 10000, 13000, -40000, -40000, -40000]):
		
		self.ppc['gencost'][0, 4] = 15.
		self.ppc['gencost'][1, 4] = 10.
		self.ppc['gencost'][2, 4] = 10.
		self.ppc['gencost'][3, 4] = 15.
		self.ppc['gencost'][4, 4] = 50.
		self.ppc['gencost'][5, 4] = 100. #generator on bus 13
		self.ppc['gencost'][:,4:] *= 100.
		self.ppc['gencost'][6:,4] = -42000
	...
	def prepare_case(self, bid_list=[7.0, 15.0, 30.0], alpha=[1.0, 1.0, 1.0, 10000000.0, 0.0], branches=[(14, 5), (25, 5), (31, 5)], costs_list = [1000, 1000, 1100, 1200, 10000, 13000, -40000, -40000, -40000]):
		self.set_bids(bid_list)
		self.fasten_branches(alpha, branches)
#		self.fasten_branches(alphas=[.5, .5, .5, .5, 5.0, 2.0], branches=[(9, 5), (23, 5), (33, 5), (39, 5), (9,5 )])
