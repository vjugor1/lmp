...
class case_2hrs:
	...
	def fasten_branches(self, alpha=0.1, branches=[(14,5), (25, 5), (31, 5)]):
		"""
		Input:
	...
	def set_costs(self):
		self.ppc['gencost'][:,4][: 6] = 400
		self.ppc['gencost'][:,4][6: ] = -10
		self.ppc['gencost'][:,4][-2: ] = -10
		...
		self.ppc['gencost'][5, 4] = 30.
		self.ppc['gencost'][:,4:] *= 100.

	def prepare_gen_cost(self):
	...
	def get_price_formings(self, verbose=True):
		if self.results == None:
			...
			return is_price_forming(self.results['gen'], self.results['gencost'], verbose=verbose, Nhrs=2, results=self.results)
	...
	def prepare_case(self, bid_list=[7.0, 15.0, 30.0], alpha=1.0, branches=[(14, 5), (25, 5), (31, 5)], Nhrs=2, mul_dem=2., mul_gen=2.7, bounds_int=(-0.0004, 4.), bounds_ram=(-0.1, 0.1) ):
		self.set_bids()
		self.fasten_branches(alpha, branches)
		self.prepare_gen_cost()

		self.ppc_2hrs = generate_Nhrs_cases(self.ppc, Nhrs, mul_dem, mul_gen)
		self.ppc_2hrs = set_hours_constr(self.ppc_2hrs, bounds_int, bounds_ram, Nhrs)
